...
type LearningWatcherModule = {
  ...
  replayActions: (actions: Array<Record<string, unknown>>, values: Record<string, string>) => Promise<{ executed: number; skipped: number }>;
};
...
export function replayLearningActions(actions: Array<Record<string, unknown>>, values: Record<string, string>) {
  if (!native) throw new Error('Learning watcher is only available on Android.');
  return native.replayActions(actions, values);
}
